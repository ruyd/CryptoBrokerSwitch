...
namespace Piggy 
{
    ...
    public class AES
    {
        ...
        private const string aesPassPhrase = "3HR3z2@@9"; //can be any string
        private const string aesInitVector = "@1B2c3D4e5F6g7H8"; //must be 16 bytes
        #endregion
        ...
        public static string PassPhrase { get { return aesPassPhrase; } }
        #endregion
        #region Static Functions
        public static string EncryptEx(string text, string passPhrase = "")
        {
            if (string.IsNullOrEmpty(passPhrase))
                return new AES(aesPassPhrase, aesInitVector).Encrypt(text);
            return new AES(passPhrase, aesInitVector).Encrypt(text);
        }
        public static string DecryptEx(string text, string passPhrase = "")
        {
            if (string.IsNullOrEmpty(passPhrase))
                return new AES(aesPassPhrase, aesInitVector).Decrypt(text);
            return new AES(passPhrase, aesInitVector).Decrypt(text);
        }
        #endregion
    }
